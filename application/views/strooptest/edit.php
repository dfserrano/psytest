<div id="main">
	...
	<?php if (sizeof($results) > 0):?>
		...
		for ($i=0; $i<sizeof($results); $i++) {
			...
			echo "<td><a href='" . site_url("strooptest/edit_taker/" . $result['id']) . "'>" . $this->lang->line('label_edit') . "</a></td>";
			echo "<td><a href='#' class='delete-button' rel='" . $result['id'] . "'>" . $this->lang->line('label_delete') . "</a></td>";
			echo "<td><a href='" . site_url("strooptest/report_single/" . $result['id']) . "' target='_blank'>" . $this->lang->line('label_report') . "</a></td>";
			echo "</tr>";
		}?>
		</table>	
	<?php 
...
<script>
	$('.delete-button').click(function(e) {
		...
		if (answer) {
			id =  $(this).attr("rel");
			window.location.href = "<?php echo site_url("strooptest/delete")?>/" + id;
		}
	});
</script>