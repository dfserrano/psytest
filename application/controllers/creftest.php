...
class CrefTest extends CI_Controller {
	...
	public function report_single($id)
	{
		...
		$results = $this->creftest_model->get_single_test_results($id);
	
		$data['results'] = $results;
	
		$this->load->view('creftest/report_single', $data);
	}
	...
	public function edit($id)
	{
		...
		$data['results'] = $this->creftest_model->get_test_takers($id);
	
		$this->load->view('templates/header', $data);
		$this->load->view('creftest/edit', $data);
		$this->load->view('templates/footer');
	}
	...
	public function delete($id)
	{
		...
		$result = $this->creftest_model->get_test_taker($id);
		$test_fk = 0;
		if (sizeof($result) > 0) {
			$test_fk = $result[0]["test_fk"];
		} 
	
		$this->creftest_model->delete_taker($id);
	
		redirect('/creftest/edit/' . $test_fk, 'refresh');
	}
	...
	public function edit_taker($id)
	{
		...
		if ($this->input->post('name'))
		{
			...
			if($this->form_validation->run() == TRUE)
			{
				...
				$this->creftest_model->edit_taker($form_data);
					
				redirect('/creftest/edit/' . $this->input->post('test_fk'), 'refresh');
			}
		}
		...
		$this->load->view('templates/header', $data);
		$this->load->view('creftest/edit_taker', $data);
		$this->load->view('templates/footer');
	}
